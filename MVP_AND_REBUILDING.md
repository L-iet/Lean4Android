...
M5.0 Priority 1 is complete on the API-33 reference tablet. Settings → Interface persists Docked or Popup Output presentation; Build/Run reveals the selected mode, More → Show Output reopens the bounded result without rerunning, Popup survives Activity recreation with its completed text, and Docked retains its pane fraction and IME suppression behavior. Messages and Goals remain independent. Priority 2 automatic inline LSP completion is next.
