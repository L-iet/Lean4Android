...
The next required boundary is M3.1, before M4. `docs/editor/M3_1_APP_SHELL_REQUIREMENTS.md` specifies a Pydroid-inspired native application shell: filename/path top bar, Run/Play, anchored Files and More menus, an extensible drawer with a collapsible project tree plus Build/Verify actions, explicit Save without build, real Save As, safe tab Close, and an icon-only search close control. `docs/editor/M3_1_OPEN_WORKSPACE_REQUIREMENTS.md` defines the full-screen Open workspace flow for Recent/My Projects, safe project archive/folder import, and single Lean files opened as scratch projects or copied into an existing project. SAF remains an import/export boundary; Lake always runs against validated app-managed workspaces. M4 LSP document synchronization must wait until this UI/file workflow is device-proven while preserving the M3 recovery and offline process boundaries.
