...
M0 through M4 and the M5.0–M5.3 foreground product work are complete on the
API-33 reference tablet. Android1 is the production/foreground lane and is now at
M6 preparation and hardening. Android2 remains an isolated, frozen candidate while
its sole resumable Mathlib producer runs; its targeted prime-import gate passed,
but pack promotion is blocked by the measured multi-document native-memory failure
described in `docs/delivery/ANDROID2_MATHLIB_EDITOR_MEMORY_FAILURE.md`.
...
M3.3 is complete on the API-33 reference tablet. The drawer now exposes **Export project** through SAF `CreateDocument`; portable ZIP generation is deterministic, bounded, streaming, link-safe, and allowlists only Lean sources plus required metadata/configuration. Dirty projects offer Save and Export, Export saved version, and Cancel. Physical acceptance covered clean export, all dirty choices, real picker cancellation, archive membership/bytes, reimport under a fresh internal identity, a Wi-Fi-disabled build/run with expected output, tablet portrait/landscape and phone-sized drawer access, restored device state, fixture cleanup, and exact no-orphan checks.

M4 is complete on the API-33 reference tablet. The retained per-project service now owns the initialized pinned `lake serve` session, ordered/versioned buffer synchronization, UTF-16 positions, current-version diagnostics, cursor goals, hover, completion, definition, references, restart/reconnect, bounded stderr, and deterministic child cleanup. Settings → Editor exposes Auto/Right side/Bottom Goals placement; editor/output and Goals splitters are independently persisted, clamped, draggable, keyboard-operable, and accessibility-labeled. Physical acceptance proved real diagnostics, `⊢ 0 < answer`, hover, completion, LocationLink definition navigation, references, explicit restart with new process IDs, process recreation, explicit and Auto layouts, dragged-size persistence, and compact 720×1280/1280×720 behavior.

M4.1 is complete on the API-33 reference tablet. Files now distinguishes file-level **New** from **New Project**, and Open workspace uses the same accessible naming dialog. Names trim outer whitespace and normalize internal whitespace to hyphens before safe-ID validation; invalid and case-folded collisions remain inline without switching projects. The saveable dialog survives configuration recreation, successful atomic creation closes it before workspace recreation, and the existing recent-project/pinned-toolchain flow activates the generated offline project. Physical acceptance created, built, and ran normalized projects from both entry surfaces, then returned to the baseline and removed only the test fixtures.

M4.2 is complete on the API-33 reference tablet. Cursor inspection now requests the pinned `$/lean/plainTermGoal` protocol alongside tactic goals and independently renders **Goals**, **Expected type**, both, or **No goals**, with cursor/generation/document-version stale-response rejection. The Goals pane is the sole visible Lean-server status surface; the redundant status row between the editor and output panel was removed. The native editor selection toolbar preserves Android's editing actions and adds **Hover** through the same LSP path as the More menu. Hover Markdown is rendered locally as bounded selectable Compose content; Lean fences reuse the editor highlighter, other fences remain safe monospaced text, and no WebView, network load, raw HTML, or executable content is introduced. Physical acceptance proved real tactic and term states, the standard toolbar plus Hover, highlighted hover code, compact recreation, restored device state, and no orphan child.

M4.3 is complete on the API-33 reference tablet. Fresh named projects derive a safe Lean/Lake identity from capitalized, snake_case, camelCase, TitleCase, space-containing, and hyphenated user names; initial `Main.lean` is valid before the first sibling-module build. Initial sessions open Main plus at most one companion so large trees do not synchronize every file to Lean Server. The drawer has a compact left-aligned hierarchical file tree with typed folder/file icons, independent two-axis scrolling, and a separately scrollable action region. The editor has a horizontally scrollable Lean/Unicode symbol row; explicit Activity resize plus IME padding keeps it above Samsung's docked split keyboard, while floating mode retains Android's overlay behavior. Device acceptance covered large/deep trees, stable Settings bounds, real symbol insert/undo, fixture cleanup, baseline Run, and exact no-orphan checks.

M4.4 is complete on the API-33 reference tablet. Goals and Output expose accessible collapse controls on their orientation-aware splitters, Messages exposes the same control in its counted header, and saveable collapsed state remains independent of each stored pane fraction. A visible docked IME temporarily removes Output and its splitter without mutating that state, leaving the symbol row and Messages in the resized workspace; a floating IME keeps overlay behavior. An expanded drawer tree receives a 120 dp allocation when window height is less than half its width, while drawer actions retain their own vertical scrolling. Host tests cover state/threshold rules; physical acceptance covered Right/Bottom Goals placement, rotation recreation, all three panels, floating HoneyBoard, a docked IME, 1280×600 layout, restored device state, and no orphan children.

M4.5 pre-M5 UI polish is complete on the API-33 reference tablet. Goals/Output boundaries are 16 dp with attached circular collapse controls, the symbol row uses compact buttons, and collapsed Messages is 32 dp high. Drawer visibility, Project expansion, and expanded folders are saveable across configuration recreation; a closed tree no longer consumes weighted space before Build project. Messages uses neutral surface colors for warning/information/hint diagnostics and switches to the error container only when severity 1 is present. Host severity tests and physical configuration/spacing/dimension checks passed.

M4.6 is complete on the API-33 reference tablet. The drawer exposes Delete Project and Rename Project below Export project; Open workspace projects, editor tabs, and project-tree entries expose anchored Rename/Delete menus on long press. Repository operations atomically rename contained projects/files/folders, recursively delete without following links, reject traversal/case-folded collisions/self-nesting and Lean-extension loss, and preserve at least one project/source. Folder actions remap or close all affected tabs, with destructive dirty loss called out in confirmation. Active project transitions synchronously persist preferences and swap in-memory project/session/store state before recreation, preventing stale recovery or a Compose race against a removed path. Host lifecycle tests and physical active rename/delete, Ready LSP, fixture cleanup, and offline baseline Run passed.

The M5 dynamic-module corrective is physically accepted. App-managed generated projects derive deterministic Lake configuration from their source topology: every contained `.lean` path becomes a sorted exact module in `roots`, and the generated block deliberately omits `globs` so pinned Lake derives one exact default glob per root. This preserves overlapping modules such as `Foo.lean` and `Foo/Bar.lean` without Lake's bad-import failure from overlapping explicit globs. `core-project` validates Lean module path components and owns idempotent reconciliation after source lifecycle/import/save changes and before Build/Run or LSP startup; UI code only asks it for the derived `<LeanProjectName>/New.lean` default. Only the recognized generated `lean_lib` shape is migrated, so arbitrary imported Lake semantics fail explicitly instead of being silently rewritten. Host tests and an API-33 offline seven-job build/run, Ready LSP, cleanup reconciliation, and cold recreation passed.

The post-M4.6 roadmap has two coordinated lanes. The producer lane builds the pinned Android2-compatible Mathlib artifacts incrementally and may take days; it gates Mathlib pack promotion but does not block independent product work. The Android1 foreground lane has completed the M5 dynamic-module corrective and M5.0–M5.3 and has entered M6 preparation/hardening. Pause it only where a change shares the producer tree, Android2 runtime/pack contract, or physical-device state required by a Mathlib checkpoint. Non-Lean text uses a plain-text fallback behind an extension/content-type highlighter registry; extra grammars, Lean-style backslash abbreviation completion, and word wrap are deferred to post-beta M7.

The Android2 basic-prime editor gate now has a measured memory boundary. One active
`Mathlib.Data.Nat.Prime.Basic` document reaches Ready on the API-33/6 GB reference
tablet, but its Lean worker retains about 1.85 GB; recovery that opens three Mathlib
documents creates three workers and is terminated by LMKD under system pressure.
Do not diagnose this from the Compose/app PSS alone, and do not use the crash-prone
multi-tab fixture for routine proof checks. Until the lifecycle correction is device
validated, cold-launch a one-file project, wait for Ready/No goals, and monitor the
entire package UID process tree plus `MemAvailable`, swap, PSI, logcat LMKD, and
`dumpsys activity exit-info`. See
`docs/delivery/ANDROID2_MATHLIB_EDITOR_MEMORY_FAILURE.md` for the exact evidence and
promotion requirements.

M5.0 is complete on the API-33 reference tablet. Settings → Interface persists Docked or Popup Output presentation; Build/Run reveals the selected mode, More → Show Output reopens the bounded result without rerunning, Popup survives Activity recreation with its completed text, and Docked retains its pane fraction and IME suppression behavior. Settings → Editor persists automatic completion; Ready-LSP typing opens at most five caret-adjacent candidates above the IME, and touch or keyboard selection applies one safe history edit without placing completion text in Goals.

M5.1 is complete on the API-33 reference tablet. Supported general text files use a contained plain-text fallback while Lean files retain Lean-specific behavior. Programs run from a deterministic app-managed project working directory with explicit immediate-EOF, interactive-console, or selected-project-file stdin modes. Interactive Send/EOF/Cancel and Activity recreation preserve service-owned process/input state; selected saved revisions stream exact bounded bytes with dirty Save and Run/Run saved version choices. Stdout and stderr are captured independently with truncation metadata and exported byte-faithfully through separate SAF actions. Physical offline acceptance covered exact project input, interactive lifecycle, distinct stream export, picker cancellation, cleanup, and no orphan children. The detailed ownership, backpressure, lifecycle, UI, security, and validation design remains in `docs/project/STDIN_SUPPORT.md`; prompts remain ordinary output and are never treated as a structured input-request signal.

M5.2 is complete on the API-33 reference tablet. Definition and reference Location/LocationLink results pass through one typed boundary that permits navigation only to current visible contained project files, preserves truthful disabled URI/position display for inaccessible targets, rejects malformed/negative/stale generation/version/cursor results, and caps sorted/deduplicated references at 100. Go to definition opens and focuses the target UTF-16 position directly; Find references uses a bounded accessible dismissible chooser whose rows navigate without losing dirty tabs. Ready Lean editors also expose Hover, Definition, and References through the native long-press menu; the API-33 Samsung toolbar has no overflow for later Compose entries, so that menu reserves its four visible slots for Copy and those three Lean actions. Physical acceptance proved real `VisualProbe.answer` definition navigation, two reference targets, row selection, identical-result reopening, Back dismissal, and no orphan child.

M5.3 is complete on the API-33 reference tablet. Settings persist independent editor-font choices of 12/14/16/18/20/22 sp and interface scaling of 85/100/115/130 percent, apply them immediately, and preserve pane/layout/IME behavior across recreation. Settings → About reports copyable app/package/build identity, active toolchain/schema, and actually probed Lean/Lake versions, with failures shown explicitly. The shared Auto Goals portrait IME correction is physically accepted in Android1 and the frozen Android2 baseline. The next foreground boundary is Android1-only M6 preparation/hardening, including the Mathlib memory-aware active-document lifecycle before feature freeze and broader API coverage.

## 9. Recovery and operational cautions

- Check Mathlib producer truth with `mathlib/scripts/status-android2-build.sh`; a stale `running` state or PID is not evidence of a live process. If the lock is free, rerunning the same documented target audits narrow damage and resumes finalized work with `--rehash --no-cache`. Never launch a concurrent producer or clean its build roots merely to restart it.
