...
The post-M4.6 roadmap has two coordinated lanes. The producer lane builds the pinned Android2-compatible Mathlib artifacts incrementally and may take days; it gates Mathlib pack promotion but does not block independent product work. The foreground lane proceeds through the M5 dynamic-module corrective, M5.0, M5.1 general project files/program streams, M5.2 source navigation, M5.3 editor/interface preferences, and M6 preparation, pausing only where a change shares the producer tree, Android2 runtime/pack contract, or physical-device state required by a Mathlib checkpoint. M5.1 gives supported programs a deterministic app-managed project working directory and explicit immediate-EOF, interactive-console, or project-file stdin modes, with separately bounded/exportable stdout and stderr. A native child merely blocks when reading stdin and prompts are ordinary output bytes, so implementation and validation must not depend on detecting a structured "input requested" event. The detailed stream ownership, backpressure, lifecycle, UI, security, and validation design is in `docs/project/STDIN_SUPPORT.md`. Non-Lean text uses a plain-text fallback behind an extension/content-type highlighter registry; extra grammars, Lean-style backslash abbreviation completion, and word wrap are deferred to post-beta M7.
