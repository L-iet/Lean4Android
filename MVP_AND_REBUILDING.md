...
The M5 dynamic-module corrective is host-implemented but still awaits physical acceptance. App-managed generated projects now derive deterministic Lake `roots` and `globs` from their source topology: every project-root Lean file is both an explicit root and glob, while every project-root module directory contributes a sorted `.*` glob. `core-project` validates Lean module path components and owns idempotent reconciliation after source lifecycle/import/save changes and before Build/Run or LSP startup; UI code only asks it for the derived `<LeanProjectName>/New.lean` default. Only the recognized generated `lean_lib` shape is migrated, so arbitrary imported Lake semantics fail explicitly instead of being silently rewritten.
