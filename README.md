...
Current implementation priorities are installer-owned Lean/Lake compatibility-link refresh, a typed deterministic process environment (including an app-writable `TMPDIR`), richer LSP diagnostics testing, offline/API-level coverage, and the core-toolchain delivery spike.
